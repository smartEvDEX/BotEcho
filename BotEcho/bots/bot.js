...
class MeetingBot extends ActivityHandler {
    constructor() {
                    ...
                    if(file.contentType == "text/html"){
                        ...
                        if( text2 != null){
                            ...
                            if(tokens > 2048){
                                ...
                                await context.sendActivity("The text is too long, I can't summarize it.");
                            } else {
        ...
        this.onMembersAdded(async (context, next) => {
            ...
            await context.sendActivity("Hi, I'm the bot."); 
            await context.sendActivity("If you send me the transcription file of the meeting I will send you the summary."); 
            await context.sendActivity("Remembre I only accept .vtt or .txt files."); 

            ///var id_meeting = context.activity.conversation.id;
            //var id_meeting = context.meetingId;
            //await context.sendActivity("First write login to have access to the options. Always wait to the answer you are logged in to ask for the summary of the meeting.");
            //await context.sendActivity("Write, if you want to receive the transcript of this meeting, at the end send me the following message: \'transcription - " + id_meeting + "\'");
        ...
        this.onDialog(async (context, next) => {
            ...
            //await context.sendActivity("onDialog"); 
            await next();
        });

        
        this.onEndOfConversation(async (context, next) => {
            await context.sendActivity("onEndOfConversation"); 
            await next();
        });
        ...
        this.onTurn(async (context, next) => {
            //await context.sendActivity("onTurn"); 
            await next();
        });
        ...
        this.onUnrecognizedActivity(async (context, next) => {
            await context.sendActivity("onUnrecognizedActivityType"); 
            await next();
        }).catch((res) => {
            return res;
        });
